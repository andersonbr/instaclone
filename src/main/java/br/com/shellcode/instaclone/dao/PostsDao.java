...
public class PostsDao extends AbstractGenericDao<Posts> {
	...
	public List<Posts> list() {
		TypedQuery<Posts> q = getEm().createQuery("select p from Posts p", Posts.class);
		return q.getResultList();
	}

	public List<Posts> listEntre(Date dtInicio, Date dtFim) {
		TypedQuery<Posts> q = getEm().createQuery("select p from Posts p where p.data between :inicio and :fim",
				Posts.class);
